...
if st.session_state.phase == "tutorial":
    ...
    if st.button("Fertig mit dem Tutorial", key=f"next_event_{st.session_state.current_round}"):
    ...
    event_text = event_manager.get_event(st.session_state.events_json, st.session_state.game_data_json, st.session_state.current_round)
    st.write(event_text)
    st.text("\n")
