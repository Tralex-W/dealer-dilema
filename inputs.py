...
class Input:
    def __init__(self):
        ...
        if 'input_mode' not in st.session_state: #gibt man ob man löscht oder hinzufügt
            st.session_state.input_mode = True

    def input_start_values(self): #Eingabe der Anzahl der Spieler
        num = st.number_input("Anzahl der Spieler:", min_value=2, step=1, value=2, max_value=6)
            ...
            for j in range(3):
                ...
                if cols[j].button(button_label, key=idx): #Ein Feld wurde angeklickt
                    ...
                    if st.session_state.input_mode == True: #Hinuzufügen
                        st.session_state.field.add_figure_to_field(idx, st.session_state.current_player, st.session_state.current_round)

                        # Speichere den Zug in der Historie (Feld und Spieler)
                        st.session_state.history.append((idx, st.session_state.current_player, st.session_state.current_round))

                    elif st.session_state.input_mode == False: #Löschen
                        st.session_state.field.remove_figure_from_field(idx, st.session_state.current_player, st.session_state.current_round)
    ...
    def redo_button(self):
        ...
        if st.button("Redo"):
            if st.session_state.history:
                ...
                if player > 0: #es handelt sich um ein hinzufügen, dass rückgänig gemacht wird
                    st.session_state.field.remove_last_from_field(idx)  # Entferne Figur vom Feld
                elif player < 0: #es handelt sich um ein löschen, dass rückgänig gemacht wird
                    st.session_state.field.add_figure_to_field(idx, abs(player), history_round)
    ...
    def input_round_values(self): #eingabe auf welchem Felder welcher Spieler steht
        ...
        if st.button("Nächster Spieler"):
            st.session_state.current_player = (st.session_state.current_player % st.session_state.number_of_players) + 1
